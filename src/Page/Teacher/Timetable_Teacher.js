...
import {
  TimetableAPIServiceTeacher,
  TimetableAPIServiceStaff,
} from "../../Service/TimetableAPIService";
    ...
    return (
      <div>
        ...
        <ManagementBox
          ...
          dataTimetable={this.state.dataTimetable}
        />
      </div>
    );
  }
...
function ManagementBox(props) {
  ...
  const handleChangeDayOfWeek = (data) => (event) => {
    ...
    TimetableAPIServiceStaff.getStartTimeOption(
      data.years,
      data.semester,
      data.course_id,
      data.course_type,
    ...
    TimetableAPIServiceStaff.getEndTimeOption(
      data.years,
      data.semester,
      data.course_id,
      data.course_type,
  ...
  useEffect(() => {
    ...
    setFilteredData(
      searchValue === ""
        ? props.dataTimetable
        : props.dataTimetable.filter((data) => {
      ...
      return (
        <>
          ...
          <TableCell align="left">
            ...
            <CardSelect
              labelPara="สิ้นสุด"
              menuItemPara={timeEndOptions}
              onChangePara={handleChangeTimeEnd(row)}
              valuePara={timeEndSelected}
            />
          </TableCell>
        </>
      );
  ...
  const clean = (dataInside) => () => {
    TimetableAPIServiceStaff.clean(
      dataInside.years,
      dataInside.semester,
      dataInside.course_id,
      dataInside.course_type,
      ...
      dataInside.end_time,
      dataInside.room_id,
      dataInside.time_locker,
      dataInside.room_locker
    ).then((res) => {
      props.updateState();
    });
  };
              ...
              <Grid container spacing={2} sx={{ pt: 0, pb: 3, pl: 3, pr: 3 }}>
                ...
                <Grid item sm={6} xs={6}>
                  <Box
                    dir="rtl"
                    spacing={2}
                    sx={{ pt: 2, display: "flex", alignItems: "flex-end" }}
                  >
                        ...
                        return (
                          <TableRow key={row.id}>
                            ...
                            <TableCell width="15%" align="left">
                              {row.member.map((inMember) => {
                                return (
                                  <TableRow key={inMember.member_id}>
                                    {" "}
                                    {inMember.member_name}{" "}
                                  </TableRow>
                                );
                              })}
                            </TableCell>
