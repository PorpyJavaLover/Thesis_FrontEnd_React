...
function CreationBox(props) {
  ...
  const handleSubmit = (event, data) => {
    NotTeachAPIServiceStaff.createNotTeach(
      dayOfWeekSelected,
      timeStartSelected,
      timeEndSelected,
  ...
  return (
    <Container
      maxWidth="false"
      sx={{ pt: 2, pb: 2, display: disableState ? "none" : "block" }}
    >
      <Card sx={{ boxShadow: 5 }}>
        <CardHeader
          title={props.title}
        ...
        <Grid container spacing={2} sx={{ pt: 2, pb: 3, pl: 3, pr: 3 }}>
          <Grid item sm={4} xs={12}>
            <CardSelect
              labelPara="วันที่ไม่สะดวกสอน"
              menuItemPara={dayOfWeekOptions}
              onChangePara={handleChangeDayOfWeek}
              valuePara={dayOfWeekSelected}
            />
          </Grid>
          <Grid item sm={4} xs={12}>
            <CardSelect
              labelPara="เลือกเวลาเริ่ม"
              menuItemPara={timeStartOptions}
              onChangePara={handleChangeTimeStart}
              valuePara={timeStartSelected}
            />
          </Grid>
          <Grid item sm={4} xs={12}>
            <CardSelect
              labelPara="เลือกเวลาสิ้นสุด"
              menuItemPara={timeEndOptions}
              onChangePara={handleChangeTimeEnd}
...
function MenagementBox(props) {
  ...
  const handleConfirm = (dataInside) => () => {
    NotTeachAPIServiceStaff.updateNotTeach(
      dataInside.notId,
      dayOfWeekSelected,
      timeStartSelected,
      timeEndSelected
  ...
  return (
    <Container
      maxWidth="false"
      sx={{ pt: 2, pb: 2, display: props.disableState ? "none" : "block" }}
    >
      <Card sx={{ boxShadow: 5 }}>
        <CardHeader
          title={props.title}
